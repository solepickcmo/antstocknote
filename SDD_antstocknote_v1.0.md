...
│   ├── tierStore        (구독 등급 및 기능 권한 제어, PERMISSIONS 맵)
│   ├── layoutStore      (반응형 레이아웃 관리 — isMobileMode)
│   ├── themeStore       (라이트/다크 테마 — persist 미들웨어 적용)
...
| HoldingsPage | /holdings | is_open=TRUE 종목 목록 + 비중 PieChart | useHoldingsSelector |
| AnalysisPage | /analysis | 전략/감정 태그별 승률 분석 + 오답노트 | tradeStore |
| CalculatorPage | /calculator | 투자 시뮬레이터 4종 | tradeStore |
