...
| 2.3.0 | 2026-04-16 | — | **Apple 디자인 시스템 연동 설계**: `DESIGN.md` 기반의 바이너리 테마, 글래스모피즘, SF Pro 폰트 시스템 및 피드백 인터랙션 상세 설계 반영 |
...
| CalculatorPage | /calculator | 물타기 시뮬레이션 계산기 | ✅ 완료 |
...
| DD-012 | **RLS 보안 고도화 및 강제 타입 캐스팅(::uuid) 적용** | 기본 연산자 비교 | DB 컬럼(TEXT)과 SDK 식별자(UUID) 간의 타입 불일치 에러를 방지하고, 프로젝트 전역의 데이터 격리 무결성을 보장하기 위해 모든 RLS 정책에 명시적 캐스팅 적용. |
| DD-015 | **Apple Human Interface 기반 UI 마이그레이션** | 기존 다크 블루 테마 | 프리미엄 이미지 구축 및 시각적 피로도 감소를 위해 Pure Black/Light Gray 바이너리 테마와 글래스모피즘을 채택함. |
| DD-016 | **글래스모피즘(Glassmorphism) 기반 깊이 설계** | 단순 그림자(Shadow) | 레이어 간의 계층 구조를 명확히 하고 고급스러운 질감을 제공하기 위해 `backdrop-filter`와 투명도를 결합한 유리 효과를 표준화함. |
