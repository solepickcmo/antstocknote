...
| 작성일 | 2026-04-09 |
| 최종 수정일 | 2026-04-22 |
| 문서 상태 | Version: v4.0.0 (2026-04-22) - Status: 구현 완료 (Feature 1~3 대규모 기능 확장) - 업데이트 사항: 투자 원칙(PrinciplesPage), 저평가 우량주 시뮬레이터(QualityStockEvaluator), 종목 분석 기록(StockAnalysisPage) 풀스택 설계 반영 |
| 적용 범위 | 매매일지 앱 전체 시스템 (MVP + Premium Tier + Feature 확장 범위) |
| 참조 SRS | SRS_매매일지앱_v4.0 (IEEE 830-1998) |
| 설계 언어 | UML 2.x (텍스트 표현), SQL DDL, Supabase SDK, Express REST API |
...
| 4.0.0 | 2026-04-22 | — | **대규모 기능 확장 (Feature 1~3)**: ① 저평가 우량주 시뮬레이터(`QualityStockEvaluator`) 추가 및 `ValuationCalculator` 대체. ② 투자 원칙 페이지(`PrinciplesPage`) + TradeModal 원칙 확인 게이트(`PrincipleCheckModal`) 풀스택 구현. ③ 종목 분석 기록(`StockAnalysisPage` + `StockAnalysisModal`) 풀스택 구현 및 HoldingsPage 연동. 백엔드 `investment_principles`, `stock_analyses` 테이블 및 RESTful API 신규 설계. |
---
...
현재 구현은 **프론트엔드(Vercel)** + **백엔드 REST API(Railway, Express)** + **Supabase(DB + Auth)** 의 3-티어 구조로 운영된다. 프론트엔드 내부는 Pages / Components / Store / API 레이어로 분리되며, 백엔드는 Service / Controller / Routes 레이어로 분리된다.

```
┌──────────────────────────────────────────────────────────────┐
│  Pages Layer  (라우팅 단위 화면)                              │
│  ● DashboardPage, CalendarPage, HistoryPage, HoldingsPage    │
│  ● AnalysisPage, CalculatorPage, ProfilePage                 │
│  ● PrinciplesPage (투자 원칙 — Feature 1)                    │
│  ● StockAnalysisPage (종목 분석 기록 — Feature 3)            │
│  ● LoginPage, RegisterPage, ResetPasswordPage                │
│  ● AdminSubscriptionPage (관리자 전용, /admin/subscriptions) │
├──────────────────────────────────────────────────────────────┤
...
│  ● PrincipleCheckModal (매매 전 원칙 확인 게이트 — Feature 1) │
│  ● StockAnalysisModal  (종목 분석 작성/편집 — Feature 3)     │
│  ● QualityStockEvaluator (저평가 우량주 평가기 — Feature 2)  │
├──────────────────────────────────────────────────────────────┤
│  Store Layer  (Zustand 전역 상태 + 비즈니스 로직)             │
│   ├── authStore           (인증 세션 및 역할 관리)
│   ├── tradeStore          (매매/PnL 로직 — Free Tier 월 30건 제한 포함)
│   ├── tierStore           (구독 등급 및 기능 권한 제어)
│   ├── layoutStore         (반응형 레이아웃 관리)
│   ├── themeStore          (라이트/다크 테마)
│   ├── tagStore            (전략/감정 태그 목록 관리)
│   ├── goalStore           (월별 목표 손익 관리)
│   ├── principleStore      (투자 원칙 CRUD — Feature 1)
│   └── stockAnalysisStore  (종목 분석 기록 CRUD — Feature 3)
├──────────────────────────────────────────────────────────────┤
│  Lib Layer  (순수 유틸리티 함수)                              │
│  ● stockEvaluator.ts — 7개 지표 채점 순수 함수 (Feature 2)   │
├──────────────────────────────────────────────────────────────┤
│  API Layer  (Supabase SDK + 백엔드 REST 클라이언트)          │
│  ● supabase.ts — createClient 초기화 및 export               │
│  ● 백엔드 호출: fetch( VITE_API_URL + '/api/v1/*' )          │
└──────────────────────────────────────────────────────────────┘
```
...
| CalendarPage | /calendar | 날짜별 PnL 캘린더 + 월별 요약 | tradeStore |
| HistoryPage | /history | 매매 목록(DesktopHistoryView/MobileHistoryView 분기) | tradeStore |
| HoldingsPage | /holdings | is_open=TRUE 종목 목록 + 비중 PieChart + 종목별 분석 버튼 | tradeStore, StockAnalysisModal |
| AnalysisPage | /analysis | 전략/감정 태그별 승률 분석 + 오답노트 | tradeStore |
| CalculatorPage | /calculator | 투자 시뮬레이터: 복리/배당/리스크 + **저평가 우량주 평가기** | stockEvaluator.ts |
| ProfilePage | /profile | 닉네임 수정 + 회원 탈퇴 | authStore |
| AdminSubscriptionPage | /admin/subscriptions | 구독 승인 및 관리 | authStore, tierStore |
| **PrinciplesPage** | **/principles** | **투자 원칙 최대 5개 작성/수정/삭제** | **principleStore** |
| **StockAnalysisPage** | **/stock-analysis** | **종목 분석 기록 목록 + 검색 + Ticker 필터** | **stockAnalysisStore** |
...
#### 7.1 subscriptions (구독 정보)
| 컬럼명 | 타입 | 제약조건 | 설명 |
|---|---|---|---|
| user_id | UUID | PK, FK → auth.users.id | 유저 식별자 |
| tier | TEXT | free / premium | 구독 등급 (Basic 폐기됨) |
| status | TEXT | active/pending/expired/canceled | 승인 상태 |
| expires_at | TIMESTAMPTZ | NULL 허용 | 만료 일시 (NULL 시 무제한) |
...
#### 7.3 investment_principles (투자 원칙 — Feature 1)

> 유저별 최대 5개의 투자 원칙을 관리한다. `user_id + order_num` 복합 유니크 제약으로 순번 중복을 방지한다.

| 컬럼명 | 타입 | 제약조건 | 설명 |
|---|---|---|---|
| id | UUID | PK, DEFAULT gen_random_uuid() | 원칙 고유 ID |
| user_id | TEXT | NOT NULL, INDEX | 유저 식별자 |
| order_num | INT | NOT NULL, UNIQUE(user_id) | 원칙 순서 (1~5) |
| content | TEXT | NOT NULL | 원칙 내용 |
| created_at | TIMESTAMPTZ | DEFAULT NOW() | 생성일시 |
| updated_at | TIMESTAMPTZ | AUTO UPDATE | 수정일시 |

**설계 결정**: Prisma `upsert` 방식으로 배열 통째 저장하여 순서 변경·삭제·추가를 단일 PUT 요청으로 처리. 개별 PATCH는 UX 복잡도 증가 및 N+1 요청 문제를 야기하므로 채택하지 않음.

#### 7.4 stock_analyses (종목 분석 기록 — Feature 3)

> 종목별 자유형식 분석 본문을 저장한다. ticker는 nullable로 보유 여부와 관계없이 기록 가능하다.

| 컬럼명 | 타입 | 제약조건 | 설명 |
|---|---|---|---|
| id | UUID | PK, DEFAULT gen_random_uuid() | 분석 고유 ID |
| user_id | TEXT | NOT NULL, INDEX | 유저 식별자 |
| ticker | TEXT | NULL 허용 | 종목 코드 (미선택 가능) |
| stock_name | TEXT | NULL 허용 | 종목명 |
| title | TEXT | NOT NULL | 분석 제목 |
| content | TEXT | NOT NULL | 분석 본문 (plain text) |
| analysis_date | TIMESTAMPTZ | DEFAULT NOW() | 분석 기준 일시 |
| created_at | TIMESTAMPTZ | DEFAULT NOW() | 생성일시 |
| updated_at | TIMESTAMPTZ | AUTO UPDATE | 수정일시 |
...
CREATE INDEX idx_investment_principles_user_id ON investment_principles(user_id);
...
CREATE INDEX idx_sa_user_ticker ON stock_analyses(user_id, ticker);
CREATE INDEX idx_sa_user_date ON stock_analyses(user_id, analysis_date DESC);
```
...
### 8.9 투자 원칙 API (Feature 1)

> 백엔드 REST API (`/api/v1/principles`). JWT 인증 필수.

| 메서드 | 경로 | 설명 | 요청 바디 | 응답 |
|---|---|---|---|---|
| GET | `/api/v1/principles` | 내 원칙 목록 조회 (order_num ASC) | — | `InvestmentPrinciple[]` |
| PUT | `/api/v1/principles` | 원칙 배열 전체 upsert 저장 | `{ principles: [{order_num, content}] }` | `InvestmentPrinciple[]` |
| DELETE | `/api/v1/principles/:id` | 단일 원칙 삭제 | — | 204 No Content |

**설계 결정 (DD-015)**: 원칙 저장 시 PUT + 배열 upsert 방식 채택.
- 이유: 사용자가 여러 원칙을 편집 후 한 번의 저장 버튼으로 처리하는 UX 지원
- 대안 비교: 개별 PATCH는 N개 원칙 × N번 요청으로 네트워크 낭비 발생

**TradeModal 원칙 확인 게이트 흐름**:
```
저장 버튼 클릭
  → handleSaveClick() (유효성 검사)
  → PrincipleCheckModal 오픈
    ├── 원칙 있음: 목록 표시 → "원칙에 따라 거래했습니다" 확인
    └── 원칙 없음: 원칙 작성 페이지 이동 or 건너뛰기
  → onConfirm() → handleSubmit() (실제 저장)
```

### 8.10 저평가 우량주 시뮬레이터 (Feature 2)

> 순수 프론트엔드 로직. 백엔드 API 없음. `src/lib/utils/stockEvaluator.ts`에 구현.

**평가 지표 및 채점 기준 (총 14점 만점)**:

| 지표 | 2점 기준 | 1점 기준 | 0점 기준 |
|---|---|---|---|
| PER | 10 미만 | 10~20 | 20 초과 또는 음수 |
| PBR | 1 미만 | 1~2 | 2 초과 |
| ROE | 15% 이상 | 10~15% | 10% 미만 |
| ROIC | 15% 이상 | 10~15% | 10% 미만 |
| 영업이익률 | 10% 이상 | 5~10% | 5% 미만 |
| 부채비율 | 100% 미만 | 100~200% | 200% 초과 |
| 배당수익률 | 3% 이상 | 1~3% | 1% 미만 |

**등급 판정**: 11~14점 → 🟢 저평가 우량주 / 7~10점 → 🟡 적정 / 0~6점 → 🔴 고평가

**설계 결정 (DD-016)**: 평가 로직을 UI와 완전 분리한 순수 함수(`stockEvaluator.ts`)로 구현.
- 이유: 독립 테스트 가능성 확보, 지표 추가/변경 시 UI 수정 없이 로직만 수정
- `evaluateStock(metrics: StockMetrics): EvaluationResult` 단일 진입점 제공

### 8.11 종목 분석 기록 API (Feature 3)

> 백엔드 REST API (`/api/v1/stock-analyses`). JWT 인증 필수.

| 메서드 | 경로 | 설명 | 요청 바디 | 응답 |
|---|---|---|---|---|
| GET | `/api/v1/stock-analyses` | 전체 목록 조회 (ticker 쿼리 파라미터 필터) | — | `{ total, page, analyses[] }` |
| GET | `/api/v1/stock-analyses/ticker/:ticker` | 특정 종목 분석 목록 | — | `StockAnalysis[]` |
| POST | `/api/v1/stock-analyses` | 분석 생성 | `{ ticker?, stock_name?, title, content, analysis_date? }` | `StockAnalysis` |
| PATCH | `/api/v1/stock-analyses/:id` | 분석 수정 (제목/내용/날짜만) | `{ title?, content?, analysis_date? }` | `StockAnalysis` |
| DELETE | `/api/v1/stock-analyses/:id` | 분석 삭제 | — | 204 No Content |

**설계 결정 (DD-017)**: ticker를 PATCH 대상에서 제외.
- 이유: 종목 변경은 완전히 새로운 분석으로 의미적으로 분리되어야 함
- ticker 변경 시 기존 분석을 삭제하고 새 분석을 작성하도록 UX 유도

**HoldingsPage 연동**:
- 보유 종목 행마다 「파일텍스트 아이콘 + 분석」 버튼 추가
- 클릭 시 `StockAnalysisModal`이 해당 종목(ticker, stock_name) preset으로 오픈
- 분석 저장 후 `/stock-analysis` 페이지에서 ticker 필터로 바로 조회 가능
