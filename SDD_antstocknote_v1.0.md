...
| 2.9.2 | 2026-04-18 | — | **접근성 및 사용성 고도화**: 라이트 모드 명도 대비(사이드바/티커) 개선, 통계 데이터 정수화 처리, 시뮬레이터 탭 사용성 최적화 |
| 3.0.0 | 2026-04-19 | — | **시스템 안정화 및 정합성 설계 패치**: 부동소수점 오차 방어 전략 명세 추가, 오답노트 UX 개선(Auto-resize) 및 계산기 레이아웃 반응형 최적화 내역 반영 |
| 3.0.1 | 2026-04-19 | — | **Bugfix v2 배포 설계 반영**: 보유 종목 파이 차트 데이터 모델 연동, 계산기 결과 우측 정렬 속성 매핑, 거래 건수 기반 승률 신뢰도 판단 로직(10회 미만 표본 부족) 설계 추가 |
...
| DD-019 | **금융 데이터 부동소수점 오차 방어 전략** | 단순 비교 연산 | 전량 매도 시 잔류 수량이 미세하게 남는 현상을 방지하기 위해 8자리 반올림(`toFixed(8)`) 및 `0.000001` 임계값을 적용하여 `is_open` 상태를 결정함. |
| DD-020 | **오답노트 UX 개선을 위한 가변 높이 입력창 설계** | 고정 높이 textarea | 사용자 입력 내용의 양에 관계없이 가독성을 유지하기 위해 `useRef`와 `useEffect`를 활용하여 텍스트 길이에 따라 높이가 자동 조절되는 Auto-resize textarea를 도입함. |
