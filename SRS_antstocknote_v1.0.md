...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| FR-030 | 대시보드는 유저의 전체 통합 자산(투자원금 + 평가손익)을 표시하며, 금액 표기 시 **소수점을 제거**해야 한다. 또한 기존 '개미의 집(타운)' 영역을 제거하고 분석 탭의 내용(전략/감정 실적 등)을 삽입해야 한다. | P1 |
| FR-031 | 대시보드는 해당 유저의 기간별(1W/1M/3M/1Y) 누적 수익률 추이 차트를 표시해야 한다. | P1 |
