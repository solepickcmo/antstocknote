...
| 2.5.0 | 2026-04-16 | — | **Apple 디자인 시스템 전면 도입**: 'Pure Black/Light Gray' 바이너리 테마, SF Pro 타이포그래피, 글래스모피즘(Glassmorphism) 기반 UI 마이그레이션 반영 |
...
- 태그(전략/감정)는 시스템이 매수/매도 상황에 최적화된 고정 리스트를 제공하며, 사용자는 그 중 선택한다. (v2.4.0 롤백: 커스텀 관리 기능 제외)
- **디자인 시스템(v2.5.0)**: Apple의 디자인 가이드를 준수하며, Pure Black(`#000000`)과 Light Gray(`#f5f5f7`)의 바이너리 섹션 리듬, Apple Blue(`#0071e3`) 단일 액센트, SF Pro 기반의 정밀한 타이포그래피 계층을 필수 적용한다.
...
### 4.12 손익분기점(BEP) 계산기 (미구현)

| ID | 요구사항 | 우선순위 | 구현 상태 |
|---|---|---|---|
| FR-140 | 사용자는 분할 매수 단가와 수량을 입력하여 손익분기 매도가를 계산할 수 있어야 한다. | P2 | ✅ 완료 |
| FR-141 | 평균 매수 단가, 총 투자금액, 수수료 포함 필요 수익률을 실시간 계산하여 표시한다. | P2 | ✅ 완료 |
...
| NFR-031 | **Apple 스타일 바이너리 테마**를 지원해야 한다 (Pure Black & Light Gray). |
| NFR-032 | 앱은 iOS 15 / Android 10 이상에서 정상 동작해야 하며, 전반적인 UX는 Apple의 Human Interface Guidelines(HIG) 감성을 지향한다. |
| NFR-033 | 모든 주요 내비게이션 및 모달 배경은 반투명 유리 효과(Glassmorphism)를 적용하여 시각적 깊이감을 제공해야 한다. |
