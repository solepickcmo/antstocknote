...
| 작성일 | 2026-04-08 |
| 최종 수정일 | 2026-04-22 |
| 문서 상태 | Version: v4.0.0 (2026-04-22) - Status: 구현 완료 (Feature 1~3 대규모 기능 확장) - 목표: 투자 원칙, 저평가 우량주 시뮬레이터, 종목 분석 기록 기능 구축 완료 |
| 적용 범위 | 매매일지 앱 전체 시스템 (MVP + Subscription Management + Feature 확장 범위) |
...
| 3.1.0 | 2026-04-19 | — | **시스템 안정화 및 버그 픽스**: 부동소수점 오차 방어(is_open 업데이트), 모바일 히스토리 기록하기 버튼 복구, 오답노트 auto-resize textarea 도입, 계산기 4종 모바일 반응형 및 가독성 최적화, 대시보드-분석페이지 데이터 정합성(노트 카운트) 통일 |
| 3.1.1 | 2026-04-19 | — | **Bugfix v2 배포 반영**: 라이트모드 텍스트 대비 명확화, 보유 종목 PieChart 포트폴리오 비중 시각화, 투자 시뮬레이터 인풋 우측 정렬 개선, 분석 페이지 거래 건수 및 승률 신뢰도 표본 추가 |
| 3.2.0 | 2026-04-20 | — | **Orchestration v2.0 정렬 및 구독 관리 시스템 구축**: 티어 체계 2단계(Free/Premium) 통환, `subscriptions` 및 `profiles` 테이블 도입, 수동 승인(Pending/Active) 및 어드민(antstocknote@gmail.com) 관리 기능 구현 |
| 3.2.1 | 2026-04-20 | — | **프로필 및 계정 관리 통합**: 닉네임 수정 및 회원 탈퇴(데이터 삭제) 기능 명세 추가 |
| 4.0.0 | 2026-04-22 | — | **대규모 기능 확장 (Feature 1~3)**: ① 저평가 우량주 시뮬레이터 (FR-220~224) 구현 — 7개 지표 채점 기반 평가기, `stockEvaluator.ts` 순수 함수 구조. ② 투자 원칙 페이지 (FR-230~234) 구현 — CRUD + TradeModal 연동 원칙 확인 게이트. ③ 종목 분석 기록 (FR-240~246) 구현 — 자유형식 작성/수정/삭제 + HoldingsPage 연동. |
...
| 보유 종목 | is_open=TRUE 종목 목록, 비중 계산, 종목별 분석 버튼 | P1 (MVP) | ✅ 완료 |
| 매매 복기/분석 | 전략별 승률, 실수 유형, 오답 노트 (AnalysisPage) | P2 | ✅ 완료 |
| 설정/도구 | 투자 시뮬레이터 (복리/배당/리스크/저평가 우량주), 매매내역 내보내기 | P2 | ✅ 완료 |
| **투자 원칙 관리** | **원칙 CRUD + TradeModal 확인 게이트 (PrinciplesPage)** | **P1 (Feature 1)** | **✅ 완료** |
| **저평가 우량주 시뮬레이터** | **7개 지표 채점 기반 종목 평가기 (stockEvaluator.ts)** | **P2 (Feature 2)** | **✅ 완료** |
| **종목 분석 기록** | **자유형식 분석 작성 + HoldingsPage 연동 (StockAnalysisPage)** | **P1 (Feature 3)** | **✅ 완료** |
...
| Tier | 월 요금 | 대상 | 설명 |
|---|---|---|---|
| **Free** | 무료 | 신규 가입자 기본 | 핵심 기록 기능 제한적 제공 (월 30건). 체험 목적 |
| **Premium** | 월 6,900원 | 심층 분석·도구 활용 사용자 | 무제한 기록·전체 기간 조회·계산기·감정 분석 전체 제공 |

> **Basic Tier 폐기 (2026-04-20)**: 사용성 개선 및 아키텍처 단순화를 위해 3단계 구조에서 2단계(Free/Premium) 구조로 통합함.
...
|---|---|---|---|
| 매매 기록 | 월 30건 | 무제한 |
| 히스토리 조회 기간 | 최근 3개월 | 전체 기간 |
| 보유 종목 분석 (Holdings) | 제한적 | 전체 제공 |
| 수수료/손익 자동 계산 | O | O |
| 수익 캘린더 요약 | O | O |
| 매매 복기/분석 대시보드 | O | O |
| 투자 시뮬레이터 4종 | X | O |
...
| ID | 요구사항 | 우선순위 | 구현 상태 |
|---|---|---|---|
| FR-110 | 앱 초기화 시 `tierStore`는 `subscriptions` 테이블을 조회하여 현재 사용자의 Tier 및 상태를 로드해야 한다. | P1 | ✅ 완료 |
| FR-111 | 사용자가 Premium 신청 시 `status`는 `pending`으로 생성되며, 관리자의 승인 후 `active`로 전환되어야 한다. | P1 | ✅ 완료 |
| FR-112 | Free 사용자가 월 30건 한도 초과 시도 시 저장을 차단하고 업그레이드 안내를 표시해야 한다. | P1 | ✅ 완료 |
| FR-113 | 관리자는 `/admin/subscriptions` 경로를 통해 `pending` 유저를 승인하거나 기간을 연장할 수 있어야 한다. | P1 | ✅ 완료 |
| FR-114 | 모든 권한 체크는 `status === 'active'` 및 `expires_at` 유효성을 기준으로 수행되어야 한다. | P1 | ✅ 완료 |
...
### 4.17 저평가 우량주 시뮬레이터 (Feature 2)

> `ValuationCalculator`를 대체하여 CalculatorPage에 통합. 순수 프론트엔드 로직.

| ID | 요구사항 | 우선순위 | 구현 상태 |
|---|---|---|---|
| FR-220 | 사용자는 PER·PBR·ROE·ROIC·영업이익률·부채비율·배당수익률 7개 재무 지표를 입력할 수 있어야 한다. | P2 | ✅ 완료 |
| FR-221 | 시스템은 각 지표에 0~2점(총 14점 만점)을 부여하고 합산 점수를 계산해야 한다. | P2 | ✅ 완료 |
| FR-222 | 시스템은 점수 구간(11~14: 저평가 우량주 🟢, 7~10: 적정 🟡, 0~6: 고평가 🔴)을 기준으로 등급 판정 결과를 표시해야 한다. | P2 | ✅ 완료 |
| FR-223 | 시스템은 각 지표별 채점 근거(몇 점인지, 왜 그 점수인지)를 상세 피드백으로 제공해야 한다. | P2 | ✅ 완료 |
| FR-224 | 평가 로직은 UI 컴포넌트와 완전히 분리된 순수 함수(`stockEvaluator.ts`)로 구현하여 독립 테스트 가능성을 보장해야 한다. | P2 | ✅ 완료 |

### 4.18 투자 원칙 관리 (Feature 1)

> 매매 전 자기 점검 도구. TradeModal과 연동되어 규율 있는 매매를 유도.

| ID | 요구사항 | 우선순위 | 구현 상태 |
|---|---|---|---|
| FR-230 | 사용자는 자신만의 투자 원칙을 최대 5개까지 작성·수정·삭제할 수 있어야 한다. (PrinciplesPage) | P1 | ✅ 완료 |
| FR-231 | 원칙은 순번(order_num) 기반으로 표시되며, 드래그 없이 순번 변경이 가능해야 한다. | P1 | ✅ 완료 |
| FR-232 | 매매 기록 저장 시 시스템은 투자 원칙 확인 모달(PrincipleCheckModal)을 먼저 표시해야 한다. | P1 | ✅ 완료 |
| FR-233 | 원칙이 있는 경우: 목록을 보여주고 "원칙에 따라 거래했습니까?" 확인을 받은 후 저장한다. | P1 | ✅ 완료 |
| FR-234 | 원칙이 없는 경우: 원칙 작성 페이지로 이동 또는 건너뛰기를 선택할 수 있어야 한다. | P1 | ✅ 완료 |
| FR-235 | 원칙 저장은 PUT 단일 요청으로 전체 배열을 upsert 처리하여 N+1 요청을 방지해야 한다. | P1 | ✅ 완료 |

### 4.19 종목 분석 기록 (Feature 3)

> 보유 종목 단위 또는 독립적으로 작성할 수 있는 자유형식 분석 노트.

| ID | 요구사항 | 우선순위 | 구현 상태 |
|---|---|---|---|
| FR-240 | 사용자는 종목 코드(선택), 제목, 분석 본문, 분석 날짜를 입력하여 분석 기록을 생성할 수 있어야 한다. (StockAnalysisModal) | P1 | ✅ 완료 |
| FR-241 | 종목 코드 입력 시 앱 내장 CSV 종목 마스터를 기반으로 자동완성 드롭다운이 표시되어야 한다. | P1 | ✅ 완료 |
| FR-242 | 사용자는 기존 분석 기록을 수정(제목·내용·날짜)하거나 삭제할 수 있어야 한다. | P1 | ✅ 완료 |
| FR-243 | 분석 목록 페이지(StockAnalysisPage)에서 텍스트 검색(제목·종목명·티커) 및 티커 필터 칩으로 필터링할 수 있어야 한다. | P1 | ✅ 완료 |
| FR-244 | 분석 카드 클릭 시 인라인으로 본문이 펼쳐지는 아코디언 방식의 상세 보기를 제공해야 한다. | P1 | ✅ 완료 |
| FR-245 | 보유 종목 페이지(HoldingsPage)의 각 종목 행에 분석 작성 버튼이 표시되어야 하며, 클릭 시 해당 종목이 사전 설정된 상태로 StockAnalysisModal이 열려야 한다. | P1 | ✅ 완료 |
| FR-246 | 분석 데이터는 백엔드 RESTful API(`/api/v1/stock-analyses`)를 통해 서버에 영구 저장되어야 한다. | P1 | ✅ 완료 |
...
| FR-085 | 사용자 탈퇴 | ProfilePage | P1 |
| **FR-110~114** | **구독 Tier 관리** | **subscriptions, TierGate** | **P1** |
| **FR-120~123** | **다크/라이트 테마** | **themeStore, CSS 변수** | **P2** |
| **FR-130~132** | **매매내역 내보내기** | **trades, xlsx 라이브러리** | **P2** |
| **FR-140~144** | **투자 시뮬레이터 4종** | **CalculatorPage** | **P2** |
| **FR-200~214** | **매매 루틴 및 심리 관리** | **daily_journals** | **P1/P2 (미구현)** |
| **FR-220~224** | **저평가 우량주 시뮬레이터** | **CalculatorPage, stockEvaluator.ts** | **P2** |
| **FR-230~235** | **투자 원칙 관리** | **PrinciplesPage, PrincipleCheckModal, principleStore** | **P1** |
| **FR-240~246** | **종목 분석 기록** | **StockAnalysisPage, StockAnalysisModal, stockAnalysisStore** | **P1** |
