...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| FR-010 | 사용자는 복수의 투자 계좌(증권사명, 계좌명, 통화)를 등록할 수 있어야 한다. | P1 |
| FR-011 | 계좌는 수정 및 삭제가 가능하며, 삭제 시 연결된 매매 내역의 계좌 참조는 보존(soft delete)되어야 한다. | P1 |
| FR-012 | 계좌별 총 투자금 및 평가손익을 대시보드에 표시해야 한다. | P1 |
...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| FR-030 | 대시보드는 계좌별 총 자산(투자원금 + 평가손익)을 표시해야 한다. | P1 |
| FR-031 | 대시보드는 기간별(1W/1M/3M/1Y) 누적 수익률 추이 차트를 표시해야 한다. | P1 |
| FR-032 | 대시보드는 현재 보유 종목 비중을 파이 차트로 표시해야 한다. | P1 |

### 3.5 수익 캘린더

| ID | 요구사항 | 우선순위 |
|---|---|---|
| FR-040 | 시스템은 trades 테이블의 PnL을 traded_at 기준으로 일별 집계하여 달력 형식으로 표시해야 한다. | P1 |
| FR-041 | 달력의 각 날짜 셀은 해당일 PnL 합계를 색상(이익: 녹색, 손실: 적색)으로 표시해야 한다. | P1 |
| FR-042 | 날짜 셀 터치 시 해당일 매매 내역 목록으로 이동해야 한다. | P1 |
...
| FR-051 | 사용자는 종목 코드, 전략 태그, 감정 태그, 기간, 계좌로 필터링할 수 있어야 한다. | P1 |
...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| FR-060 | 시스템은 전략 태그별 매매 횟수, 승률(PnL > 0 비율), 평균 PnL을 집계하여 표시해야 한다. | P2 |
| FR-061 | 사용자는 특정 매매 건에 오답 노트(notes)를 작성할 수 있어야 한다. 항목: 실수 유형(mistake_type), 내용(content). | P2 |
| FR-062 | 오답 노트는 실수 유형별로 집계되어 분석 탭에 표시되어야 한다. | P2 |
| FR-063 | 분석 데이터는 별도 집계 테이블 없이 trades 및 notes 테이블 JOIN 쿼리로 실시간 산출해야 한다. | P2 |
...
| FR-084 | 커뮤니티 서비스를 중단하거나 삭제해도 메인 DB(users, accounts, trades, notes, tags 테이블)에 영향이 없어야 한다. | P3 |
...
메인 서비스는 5개의 핵심 테이블로 구성된다. 분석·캘린더·통계 데이터는 별도 테이블 없이 trades 및 notes 테이블 집계로 처리한다.
...
| 컬럼명 | 타입 | 제약조건 | 설명 |
|---|---|---|---|
| id | BIGINT | PK, AUTO_INCREMENT | 계좌 고유 ID |
| user_id | UUID | FK → users.id | 소유 사용자 |
| name | VARCHAR(100) | NOT NULL | 계좌 이름 |
| broker | VARCHAR(100) | NULL | 증권사/거래소명 |
| currency | VARCHAR(10) | DEFAULT KRW | 기준 통화 |
...
| account_id | BIGINT | FK → accounts.id | 계좌 참조 |
