...
| ID | 요구사항 | 우선순위 |
|---|---|---|
| FR-070 | 시스템은 기본 전략 태그와 감정 태그를 각각 5개씩 제공해야 한다. | P1 | ✅ 완료 |
| FR-071 | 사용자는 커스텀 태그를 생성·삭제할 수 있어야 하며, 카테고리당 최대 10개까지 관리 가능하다. | P1 | ✅ 완료 |
...
| ID | 요구사항 | 우선순위 | 구현 상태 |
|---|---|---|---|
| FR-130 | 사용자는 자신의 전체 매매내역을 CSV 및 xlsx 파일로 다운로드할 수 있어야 한다. | P2 | 🔲 미구현 |
| FR-131 | 내보내기 파일에는 체결일시, 종목, 유형, 가격, 수량, 수수료, 손익, 태그, 메모 등이 포함되어야 한다. | P2 | 🔲 미구현 |
| FR-132 | 내보내기는 클라이언트 사이드에서 처리하며, 데이터가 많을 시 로딩 상태를 표시해야 한다. | P2 | 🔲 미구현 |

### 4.12 손익분기점(BEP) 계산기 (미구현)

| ID | 요구사항 | 우선순위 | 구현 상태 |
|---|---|---|---|
| FR-140 | 사용자는 분할 매수 단가와 수량을 입력하여 손익분기 매도가를 계산할 수 있어야 한다. | P2 | 🔲 미구현 |
| FR-141 | 평균 매수 단가, 총 투자금액, 수수료 포함 필요 수익률을 실시간 계산하여 표시한다. | P2 | 🔲 미구현 |
