...
| FR-026 | 종목 입력 시 프론트엔드에 내장된 CSV 파일(all_stock_master.csv)을 직접 로드하고 파싱하여 초고속으로 검색 자동완성(Auto-Complete) 기능을 제공한다. 이를 통해 서버 통신 지연이나 네트워크 오류에 영향받지 않도록 보장한다. KRX 종목은 티커 좌측 0패딩 규칙을 따른다. | P1 |
